<template>
  <div class="content">
        ...
        <Content :style="{ padding: '0 32px' }">
          <div class="add-button">
            <Button
              ...
              icon="md-add"
              @click="add_user()"
              v-if="status === 1 && userInfo.role_id === 2"
              >添加</Button
            >
          </div>
              ...
              >
                ...
                <template slot-scope="{ row, index }" slot="operation">
                  <Icon
                    ...
                    size="22"
                    title="删除"
                    v-if="userInfo.role_id === 2 && row.role_id !== 2"
    ...
    <Modal v-model="AddModel" :mask-closable="false" width="40">
      ...
      >
        ...
        <FormItem label="确认密码" prop="passwordCheck">
          <Input
            type="password"
            v-model="formCustom.passwordCheck"
            autocomplete
          ></Input>
        </FormItem>
    ...
    <!-- 用户操作权限的Modal -->
    <Modal v-model="operationModel" :mask-closable="false" width="40">
      <p slot="header" style="color:#333;text-align:center">
        <span>用户操作权限</span>
      </p>
      <Form
        ...
        :model="formValidate"
        :label-width="80"
        v-if="app.length > 0 || config.length > 0 || L7Instance.length > 0"
      >
        <div>
          <div>
            <Checkbox
              :value="checkAllAPP"
              :disabled="userInfo.role_id === 1 ? true : false"
              @click.prevent.native="handleCheckAllAPP"
              v-if="userInfo.role_id === 2"
              ><span>APP全选</span></Checkbox
            >
            <Checkbox
              :value="checkAllAPP"
              :disabled="userInfo.role_id === 1 ? true : false"
          ...
          <FormItem label="" prop="apps" v-if="app.length > 0">
            <CheckboxGroup
              v-model="formValidate.apps"
              @on-change="checkAllGroupChangeAPP"
            >
              <Checkbox
                :label="value.app_id"
                border
        ...
        <div>
          <div>
            <Checkbox
              :value="checkAllL7"
              :disabled="userInfo.role_id === 1 ? true : false"
              @click.prevent.native="handleCheckAllL7"
              v-if="userInfo.role_id === 2"
              ><span>实例全选</span></Checkbox
            >
            <Checkbox
              :value="checkAllL7"
              :disabled="userInfo.role_id === 1 ? true : false"
          ...
          <FormItem label="" prop="l7ServerInfos" v-if="L7Instance.length > 0">
            <CheckboxGroup
              v-model="formValidate.l7ServerInfos"
              @on-change="checkAllGroupChangeL7"
            >
              <Checkbox
                :label="value.l7ServerId"
                border
                :disabled="userInfo.role_id === 1 ? true : false"
                v-for="value in L7Instance"
                style="margin-top: 4px;"
              >
        ...
        <div>
          <div>
            <Checkbox
              :value="checkAllConfig"
              :disabled="userInfo.role_id === 1 ? true : false"
              @click.prevent.native="handleCheckAllConfig"
              v-if="userInfo.role_id === 2"
              ><span>配置全选</span></Checkbox
            >
            <Checkbox
              :value="checkAllConfig"
              :disabled="userInfo.role_id === 1 ? true : false"
          ...
          <FormItem label="" v-if="config.length > 0">
            <CheckboxGroup
              v-model="formValidate.confs"
              @on-change="checkAllGroupChangeConfig"
            >
              <Checkbox
                :label="value.nginx_conf_id"
                border
                :disabled="userInfo.role_id === 1 ? true : false"
      ...
      <div
        style="text-align: center;font-size: 16px;margin: 28px 0;"
        v-if="
    ...
    async selUserPermission(id) {
      ...
      if (res.data.code === "success") {
        ...
        if (this.formValidate) {
          ...
          if (this.formValidate.l7ServerInfos) {
            if (
              this.formValidate.l7ServerInfos.length === this.L7Instance.length
            ) {
      ...
      if (this.checkAllAPP) {
        ...
        this.app.map((i) => {
          ...
          i.instanceParamList.map((v) => {
            if (this.formValidate.l7ServerInfos) {
              if (
                this.formValidate.l7ServerInfos.indexOf(v.instance_id) === -1
              ) {
                this.formValidate.l7ServerInfos.push(v.instance_id);
              }
            }
          });
        });
      ...
      if (data.length > 0) {
        this.app.map((v) => {
          if (data.indexOf(v.app_id) !== -1) {
            if (this.formValidate.confs) {
              if (this.formValidate.confs.indexOf(v.conf_id) === -1) {
            ...
            v.instanceParamList.map((x) => {
              if (this.formValidate.l7ServerInfos) {
                if (
                  this.formValidate.l7ServerInfos.indexOf(x.instance_id) === -1
                ) {
                  this.formValidate.l7ServerInfos.push(x.instance_id);
                }
              }
            });
          }
            ...
            this.app.map((i) => {
              if (this.formValidate.apps.indexOf(i.app_id) !== -1) {
                i.instanceParamList.map((v) => {
                  if (this.formValidate.l7ServerInfos) {
                    if (
                      this.formValidate.l7ServerInfos.indexOf(v.instance_id) ===
                      -1
                    ) {
                      this.formValidate.l7ServerInfos.push(v.instance_id);
                    }
