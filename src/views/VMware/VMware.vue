      ...
      <div class="content_right_scroll">
        <div class="content_header">
          <div class="header_title">
            ...
            <Button
              type="primary"
              v-if="$route.name === 'VMware-virtual' && !shareLoading"
              @click="openCopyModal"
              >克隆</Button
            >
            <!-- <span class="status">
          ...
          <div class="header_tab">
            <div
              class="tab"
              v-if="(activeAside instanceof Array) && !shareLoading"
            >
              <router-link :to="`/VMware/customization`" class="tab_item"
                >自定义虚拟机规范配置</router-link
              >
            </div>
            <div
              class="tab"
              v-if="!(activeAside instanceof Array) && !shareLoading"
            >
              <router-link :to="`/VMware/virtual`" class="tab_item"
                >虚拟机</router-link
              >
              <router-link :to="`/VMware/resourcepool`" class="tab_item"
                >资源池</router-link
              >
              <router-link :to="`/VMware/datastorage`" class="tab_item"
                >数据存储</router-link
              ><router-link :to="`/VMware/template`" class="tab_item"
                >模板</router-link
              >
              <!-- <router-link
        ...
        <Form ref="CopyForm" :model="CopyForm" :rules="ruleValidate">
          <FormItem label="资源池名称" prop="resourcePoolName">
            <Select v-model="CopyForm.resourcePoolName" clearable>
              <Option
                v-for="item in activeAside.resourcePoolNameList"
                :value="item.vm_name"
                :key="item.vm_name"
                >{{ item.vm_name }}</Option
              >
          ...
          <FormItem label="数据存储名称" prop="dataStoreName">
            <Select v-model="CopyForm.dataStoreName" clearable>
              <Option
                v-for="item in activeAside.datastoreNameList"
                :value="item.name"
                :key="item.name"
                >{{ item.name }}</Option
              >
...
export default {
  ...
  methods: {
    ...
    select(item) {
      this.CopyForm.dnsServerList = [];
      this.CopyForm.gatewayList = [];
      JSON.parse(JSON.stringify(this.customizationSpecNameList)).map((key) => {
        if (item.label === key.name) {
          this.dnsServerList = key.dnsServerList || [];
          this.gatewayList = key.gateway || [];
          this.CopyForm.subnetMask = key.subnetMask || "255.255.255.0";
            ...
            res.data.result[key][0].hostSystemList.map((item, index) => {
              ...
              if (this.activeAside.hostName === item.hostName) {
                this.VMwareSetActiveAside(
                  res.data.result[key][0].hostSystemList[index]
                );
              }
            });
          }
