<template>
    <div class="aside-wrap">
        ...
        <div style="display: flex; justify-content: center">
            <div class="add-list" @click="addModel(true,1)">
                <!--      <span>添加</span>-->
        ...
        <div class="aside-list">
            <div class="aside-list-wrap" v-if="!isEmptyObject(asideList) && !searchString">
                ...
                disable: !value.run_status,
              }" v-for="(value, key) in item" :key="value.l7ServerId">
                        <div @click="changeAside(value)">
                            ...
                            <div style="display: flex;margin-left: -4px;">
                                <div>
                                    <span>
                                        <Icon type="md-lock" size="20" color="#fb1010" style="cursor: pointer;" @click="LockAndUnlock(value)" v-if="value.lock_status" />
                                        <Icon type="md-unlock" size="20" color="#21a37a" style="cursor: pointer;" @click="LockAndUnlock(value)" v-if="!value.lock_status" />
                                    </span>
                                </div>
                                <div style="overflow: hidden;">
                                    <div style="padding-left: 12px;" class="info" v-if="value.nginxVersion">
                                        {{ value.nginxVersion }}
                                    </div>
                                    <div :class="value.ha_status ? 'ha-on' : 'ha-off'" v-if="value.ha_version !== 'null'">
                                        {{ value.ha_version }}
                                    </div>
                                </div>
                            </div>
                            <Icon type="md-more" size="20" color="#000" title="编辑" class="menu" @click.stop="editModel(value)" />
                        </div>
                        </MenuItem>
                    </Submenu>
                </Menu>
            ...
            <div class="aside-list-wrap" style="padding: 0 10px;" v-if="searchString">
                <div class="aside-item" :class="{
            active: item.l7ServerId === $route.params.L7,
            disable: !item.run_status,
          }" @click="changeAside(item)" v-for="(item, index) in filterAside" :key="item.l7ServerId">
                    <div class="title">
                    ...
                    <div style="display: flex:margin-left: -4px;">
                        ...
                        <div style="overflow: hidden;">
                            <div style="padding-left: 12px;" class="info" v-if="item.nginxVersion">
                                {{ item.nginxVersion }}
                            </div>
                            <div :class="item.ha_status ? 'ha-on' : 'ha-off'" v-if="item.ha_version !== 'null'">
                                {{ item.ha_version }}
                            </div>
                        </div>
                    </div>
            ...
            <div class="aside-list-wrap" style="text-align: center" v-if="isEmptyObject(asideList) && !searchString">
                暂无数据
            </div>
        ...
        <!-- 添加L7设备 -->
        <Modal v-model="l7_model_add" :transfer="false" :mask-closable="false" @on-visible-change="onVisibleChangeModalAppCopy" :styles="{top:0}">
            <div slot="header">
                <span v-if="edit">{{ "修改服务器配置" }}</span>
                <span v-else>{{ titleNameTabPane === 1 ? "添加NGINX服务器" : "新建实例" }}</span>
            </div>
            <div :class="{'modal-tabs-body-show':isTaskbarShow}">
                <Tabs v-model="titleNameTabPane" :animated="false" @on-click="changeTabs">
                    ...
                    <TabPane label="新建实例" :name="2" v-if="!edit">
                        <div>
                            <Form ref="CopyForm" :model="CopyForm" :rules="copyRuleValidate">
                                <div>
                                    <div style="display: flex;" v-for="(item, index) in CopyForm.vmParams">
                                        <div style="width: 400px;padding-right: 20px;margin-bottom: 20px;" :class="vmParamsNotSingle ? 'vmParams-border' : ''">
                                            <FormItem label="实例名称" :prop="'vmParams.' + index + '.vmName'" :rules="{ validator: validateCopyVMParamsVMname, trigger: 'blur' }">
                                ...
                                <transition name="fade">
                                    <div v-show="advancedSetupShow" class="advanced-setup-content">
            ...
            <div slot="footer">
                <Button class="fl" v-if="!addMoment" ghost type="error" @click.stop="delL7ServerInfo(add_l7_form.l7ServerId)">删除</Button>
                <Button @click="l7_model_add = false">取消</Button>
                <Button type="primary" :loading="modal_loading" @click="addL7('formValidate')" v-if="addMoment && titleNameTabPane === 1">确认添加</Button>
                <Button type="primary" :loading="modal_loading" @click="CopyModify('CopyForm')" v-if="addMoment && titleNameTabPane === 2">确认新建</Button>
                <Button type="primary" :loading="modal_loading" @click="updL7ServerInfo('formValidate')" v-if="!addMoment">确认</Button>
            </div>
        </Modal>
        <Modal v-model="initModal" :mask-closable="false" width="416" class-name="modal-l7-modal-initModal">
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            openNewInstance: (state) => state.L7.openNewInstance,
            activeAside: (state) => state.L7.activeAside,
            isNewInstance: (state) => state.L7.isNewInstance,
    ...
    methods: {
        ...
        //展示Model框，数据重置
        addModel(l7_model_add, titleNameTabPane) {
            this.edit = false;
            this.titleNameTabPane = titleNameTabPane;
            this.vmParamsNotSingle = false;
            this.l7_model_add = l7_model_add;
            ...
            if (this.copyData.hostSystemList !== null) {
                if (this.copyData.hostSystemList.length > 0) {
                    ...
                    if (
                        this.copyData.hostSystemList[0].datastoreNameList
                            .length > 0
                    ) {
                    ...
                    if (
                        this.copyData.hostSystemList[0].resourcePoolNameList
                            .length > 0
                    ) {
                    ...
                    if (
                        this.copyData.hostSystemList[0]
                            .virtualMachineTemplateList
                    ) {
                        if (
                            this.copyData.hostSystemList[0]
                                .virtualMachineTemplateList.length > 0
                        ) {
            ...
            if (this.copyData.customizationSpecNameList !== null) {
                if (this.copyData.customizationSpecNameList.length > 0) {
                    ...
                    if (
                        this.copyData.customizationSpecNameList[0]
                            .dnsServerList !== null
                    ) {
                        this.CopyForm.dnsServerList =
                            this.copyData.customizationSpecNameList[0]
                                .dnsServerList || []; //DNS服务列表有值时展示所有值
                    } else {
                        this.dnsServerList = [];
                        this.CopyForm.dnsServerList = [];
                    }
                    if (
                        this.copyData.customizationSpecNameList[0].gateway !==
                        null
                    ) {
                        this.CopyForm.gatewayList =
                            this.copyData.customizationSpecNameList[0]
                                .gatewayList || []; //网关列表有值时展示所有值
                    } else {
                        this.gatewayList = [];
                        this.CopyForm.gatewayList = [];
                    }
                    ...
                    this.CopyForm.domain =
                        this.copyData.customizationSpecNameList[0].domain ||
                        "localhost";
                } else {
                    ...
                    updL7ServerInfo(this.add_l7_form)
                        .then((res) => {
                            ...
                            if (res.data.code === "success") {
                                ...
                                if (
                                    this.l7_form.l7ServerId ===
                                    this.$route.params.L7
                                )
                                    return;
                                this.$router.replace(
                                    `/L7/${this.l7_form.l7ServerId}/chart`
                                );
                            } else {
        ...
        async GetVMwareInfo() {
            ...
            if (res.data.code === "success") {
                if (res.data.result) {
                    this.copyData = JSON.parse(JSON.stringify(res.data.result[Object.keys(res.data.result)[0]][0])); //深度拷贝第一个数据
                    for (let key in res.data.result) {
                        this.hostData.push(res.data.result[key][0]);
                    }
                    if(res.data.result[Object.keys(res.data.result)[0]][0].hostSystemList !== null){
                        this.hostName =
                        ...
                        this.customizationSpecNameList =
                            res.data.result[Object.keys(res.data.result)[0]][0]
                                .customizationSpecNameList || [];
                        this.datastoreNameList = res.data.result[Object.keys(res.data.result)[0]][0].hostSystemList[0].datastoreNameList || [];
                        this.resourcePoolNameList =
                            res.data.result[Object.keys(res.data.result)[0]][0]
        ...
        CopyModify(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                     ...
                     * 进行webSocket的连接
                     */
                    let webSocketUrl = config.webSocketUrl + getToken();
                    let webSocket = new WebSocket(webSocketUrl);
                    //  监听是否连接成功
                    webSocket.onopen = (msg) => {
                        if (webSocket.readyState === 1) {
                            // console.log(`webSocket连接成功`);
                            this.$Message.success({content: `正在创建新实例，请稍后...`,duration: 2});
                            this.l7_model_add = false;
                            this.updateIsTaskbarShow(true);
                            cloneVMwareInfo(this.CopyForm)
                                .then((res) => {
                                    if (res.data.code === "success") {
                                        this.$Message.success(`新建实例成功`);
                                        this.getL7AsideList();
                                        // if (this.activeAside === undefined || this.l7ServerId === undefined)
                                        //     return;
                                        // this.getL7AsideList().then((res) => {
                                        //     /* 第一次添加 */
        ...
        onVisibleChangeModalAppCopy(bool) {
            if (!bool) {
                ...
                //  如果设置为默认打开新建实例，取消默认
                if (this.openNewInstance) this.setOpenNewInstance(false);
            }
        },
        async init() {
            this.L7changeLoadingStatus(true);
            await this.GetVMwareInfo();
            await this.getL7AsideList(true).then((res) => {
                if (this.asyncOk(res)) {
            ...
            this.L7changeLoadingStatus(false);

            /**
             * 是否默认打开新建实例
             */
            if (this.openNewInstance) this.addModel(true, 2);
        },
    },
    created() {
        this.init();
