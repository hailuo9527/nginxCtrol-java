    ...
    <div>
        <div class="mainContent">
            ...
            <div class="tableContent">
                <Table :columns="columns" :data="TableData" width="815" style="margin: 0 auto;margin-top: 50px;" @on-selection-change="getSelect" :loading="loading">
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        <Icon type="ios-trash" size="22" style="cursor: pointer;" @click="removeSingle(row.id, index)" />
                    </template>
                </Table>
            </div>
        </div>
        <div class="footer">
            <Button size="large" class="commonOne" :class="changeStyle ? 'change_style' : ''" @click="changeStyle ? removeSelected() : ''">解除实例关联</Button>
            <Button size="large" class="commonOne" :class="changeStyle ? 'change_style' : ''" @click="changeStyle ? pushCheck(pushsSelectedInstance) : ''">推送选中实例</Button>
            <!-- <Button size="large" class="commonTwo" @click="pushCheck(pushAllInstance)"
        >推送所有实例</Button
      >-->
        </div>
        <Modal v-model="DeviceModal" width="50" ok-text="添加" @on-ok="addInstance" :mask-closable="false">
            <div slot="header">选择要关联的实例</div>
            <div class="main">
                <h3>
                    请添加一些关联。选择一个或多个要与此关联的实例配置
                </h3>
                <Select v-model="SelectModel" filterable multiple style="margin-top: 40px;" placeholder="选择Instances system或tag" @on-change="GetSelectValue">
                    <Option v-for="item in List" :value="item.l7ServerName" :key="item.l7ServerName">{{ item.l7ServerName }}</Option>
                </Select>
            </div>
        </Modal>
