        ...
        <div class="config_table_wrapper">
            <Table :columns="tableConfig" :loading="loading" @on-row-click="goConfigDetail" :data="tableData">
                <template slot-scope="{ row, index }" slot="action">
                    ...
                    <Icon type="md-lock" size="22" style="cursor: pointer; color: #f16643" v-if="row.lock_status" @click.stop="LockAndUnlock(row)" />
                    <Icon type="md-unlock" size="22" style="cursor: pointer; color: #01c864" v-if="!row.lock_status" @click.stop="LockAndUnlock(row)" />
                </template>
            </Table>
        </div>
        <Modal v-model="importModal" :mask-closable="false" width="40">
            <div slot="header">导入配置文件</div>
