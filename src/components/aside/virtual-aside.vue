<template>
  <div class="aside-wrap">
            ...
            <MenuItem
              ...
              :class="
                value.hostName === activeAside.hostName && asideIndex === 0
                  ...
                  : ''
              "
              v-for="(value, key) in item.hostSystemList"
              :key="key"
            >
    ...
    <Modal v-model="loginModal" width="40" :mask-closable="false">
...
<script>
import { insVMware, selVMwareInfo } from "@/api/VMware";
      ...
      if (res.data.code === "success") {
        if (res.data.result) {
          ...
          for (let key in res.data.result) {
            this.VMwareSetActiveAside(
              res.data.result[key][0].hostSystemList[0]
            );
            break;
          }
          for (let key in res.data.result) {
            if (res.data.result[key][0].hostSystemList) {
