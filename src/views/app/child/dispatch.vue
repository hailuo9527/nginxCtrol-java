    ...
    <div class="dispatch">
        <div class="table-wrap">
            ...
            <div class="table">
                <Table stripe :loading="loading" max-height="500" border :columns="columns" :data="dispatch.data">
                    ...
                    <template slot-scope="{ row, index }" slot="action">
                        <Button type="warning" size="small" style="margin-right: 5px" @click="modify(row, index)">修改</Button>
                    </template>
                </Table>
                <!--<Card :dis-hover="true">

        </Card>-->
                <div class="advanced" @click="showAdvance = !showAdvance" :class="{active: showAdvance}">高级
                    <Icon class="icon" size="20" type="md-arrow-dropdown" />
                </div>
                    ...
                    <div v-show="showAdvance">
                        <Table stripe :loading="advanceloading" max-height="500" border :columns="advanceColumns" :data="advanceData">
                            <div slot="header" style="text-align: right">
                                <Button type="info" style="margin-right: 5px" @click="newAdvance">新建</Button>
                            </div>
                            ...
                            <template slot-scope="{ row, index }" slot="action">
                                <Button type="warning" size="small" style="margin-right: 5px" @click="advanceModify(row, index)">修改</Button>
                                <Poptip confirm :transfer="true" title="确定要删除这一项吗?" @on-ok="advanceRemove(row, index)">
            ...
            <div class="save-bottom">
                <Button type="primary" :disabled="!(hasNotSavedAdvance || hasNotSavedDispatch)" :loading="saveLoading" @click="save">保存</Button>
            </div>
        </div>
        <Modal v-model="modal" :mask-closable="false" width="40">
            <div slot="header">修改</div>
            <div slot="close" style="margin-top: 4px;margin-right: 4px;">
                <Icon type="md-close" style="color: #fff" size="22" @click="cancel()" />
            </div>
            <div class="modal_content" v-if="modal">
                <Form ref="formDynamic" :model="dispatch" label-position="left" :label-width="70">
                    <FormItem label="修改权重" :rules="{ validator: validaterule }" :prop="'data.' + index + '.weight'">
                        <Input v-model="dispatch.data[index].weight" placeholder="权重，数值越大权重越大"></Input>
                    </FormItem>
                    <FormItem label="备份">
        ...
        <Modal v-model="advanceModal" fullscree :styles="{top:0}" width="90">
            <div slot="header">{{modifyMode? '修改高级调度' : '新建高级调度'}}</div>
                ...
                <Row class="advance-card">
                    ...
                    <Form hide-required-mark ref="formValidate" :model="advanceForm" :rules="advanceValidate">
                        <Card class="card" :dis-hover="true">
                            <p slot="title">端口</p>
                            <FormItem prop="listen_part" :rules="{required: true, message: '不能为空，没有可用端口无法创建高级调度', trigger: 'blur'}">
                                <Row>
                                    <Col span="20">
                        ...
                        <Card class="card" :dis-hover="true" v-if="advanceForm.type === 'url'">
                            <p slot="title">条件</p>
                            <Row v-for="(item, index) in advanceForm.dispatchParams" :key="index" style="margin-bottom: 10px">
                                <Col span="20" :class=" advanceForm.dispatchParams.length>1 ? 'dispatch-form-main': ''">
                                <Row>
                                    <FormItem label="URL" :prop="'dispatchParams.' + index + '.url'" :rules="{required: true, message: '不能为空', trigger: 'blur'}">
                                    ...
                                    <FormItem label="服务器名" :prop="'dispatchParams.' + index + '.upstream_name'" :rules="{required: true, message: '不能为空，没有服务器时请添加服务器', trigger: 'change'}">
                        ...
                        <Card class="card" :dis-hover="true" v-if="advanceForm.type === 'ippart'">
                            ...
                            <div v-if="advanceForm.type === 'ippart'">
                                <Row style="margin-bottom: 10px" v-for="(item, index) in advanceForm.dispatchParams" :key="index">
                                    ...
                                    <Row class="flex-box">
                                        <Col span="11">
                                        <FormItem label="起始IP" :prop="'dispatchParams.' + index + '.startIp'" :rules="{validator: ipRule}">
                                            <Input v-model.trim="item.startIp" placeholder="起始IP"></Input>
                                        </FormItem>
                                        </Col>
                                        <Col span="2" style="text-align: center; color: #666">--</Col>
                                        <Col span="11">
                                        <FormItem label="结束IP" :prop="'dispatchParams.' + index + '.endIp'" :rules="{validator: ipRule}">
                                            <Input v-model.trim="item.endIp" placeholder="结束IP"></Input>
                                        </FormItem>
                                        </Col>
                                    </Row>
                                    <FormItem label="服务器名" :rules="{required: true, message: '不能为空，没有服务器时请添加服务器', trigger: 'change'}" :prop="'dispatchParams.' + index + '.upstream_name'">
                        ...
                        <Card class="card" :dis-hover="true" v-if="advanceForm.type === 'cookie'">
                            ...
                            <div v-if="advanceForm.type === 'cookie'">
                                <Row v-for="(item, index) in advanceForm.dispatchParams" :key="index" style="margin-bottom: 10px">
                                    <Col span="20" :class=" advanceForm.dispatchParams.length>1 ? 'dispatch-form-main': ''">
                                    <Row>
                                        <FormItem label="KEY" :prop="'dispatchParams.' + index + '.key'" :rules="{required: true, message: '不能为空', trigger: 'blur'}">
                                            <Input v-model="item.key"></Input>
                                        </FormItem>
                                        <FormItem label="VALUE" :prop="'dispatchParams.' + index + '.cookie'" :rules="{required: true, message: '不能为空', trigger: 'blur'}">
                        ...
                        <Card class="card" :dis-hover="true" v-if="advanceForm.type === 'request'">
                            ...
                            <div>
                                <Row v-for="(item, index) in advanceForm.dispatchParams" :key="index" style="margin-bottom: 10px">
                                    <Col span="20" :class=" advanceForm.dispatchParams.length>1 ? 'dispatch-form-main': ''">
                                    <Row>
                                        <FormItem label="REQUEST" :prop="'dispatchParams.' + index + '.param'" :rules="{required: true, message: '不能为空，没有服务器时请添加服务器', trigger: 'change'}">
                                            <Input v-model="item.param"></Input>
                                        </FormItem>
                                        <FormItem label="服务器名" :prop="'dispatchParams.' + index + '.upstream_name'">
                                            <!--<Input v-model="item.upstream_name"></Input>-->
                                            <Select v-model="item.upstream_name">
                                                <Option v-if="i.upstream_server_name" v-for="i in advanceForm.upstreaParams" :value="i.upstream_server_name" :key="i.upstream_server_name">{{ i.upstream_server_name }}</Option>
                                            </Select>
                        ...
                        <Card :dis-hover="true">
                            ...
                            <Row style="margin-bottom: 10px" v-for="(item, index) in advanceForm.upstreaParams" :key="index">

                                <Row class="flex-box bg-gray">
                                    <Col span="20" :class=" advanceForm.upstreaParams.length>1 ? 'dispatch-form-main': ''">
                                    <Row style="margin-bottom: 20px">
                                        <FormItem label="命名" :prop="'upstreaParams.' + index + '.upstream_server_name'" :rules="{required: true, message: '不能为空', trigger: 'blur'}">
                                            <Input type="text" v-model.trim="item.upstream_server_name" placeholder="命名"></Input>
                                        </FormItem>
                                    </Row>

                                    <Row style="flex: 1; flex-basis: 30%">
                                        <FormItem label="地址" :prop="'upstreaParams.' + index + '.upstream_server_address_input'" :rules="{validator: address}">
                                            <Input type="text" @on-blur="addDomainName(index, item.upstream_server_address_input)" @on-enter="addDomainName(index, item.upstream_server_address_input)" v-model.trim="item.upstream_server_address_input" placeholder="服务器地址">
                                            </Input>
                                        </FormItem>

                                    </Row>
                                    <Row>
                                        <Button icon="md-close" class="tag" :key="i" v-if="item" @click="removeTag(index, i)" v-for="(item, i) in advanceForm.upstreaParams[index].upstream_server_address">{{item}}</Button>
                                    </Row>
...
export default {
    ...
    data() {
        ...
        this.address = (rule, value, callback) => {
            ...
            if (!value) {
                if (
                    !this.advanceForm.upstreaParams[index]
                        .upstream_server_address.length
                ) {
    ...
    methods: {
        ...
        removeTag(index, i) {
            let arr = this.advanceForm.upstreaParams[index]
                .upstream_server_address;
            arr.splice(i, 1);
        },
        addDomainName(index, str) {
            if (!this.ipAndPort.test(str)) return;
            this.advanceForm.upstreaParams[index].upstream_server_address.push(
                str
            );
            this.advanceForm.upstreaParams[
                index
            ].upstream_server_address_input = "";
        },
        typeChange(value) {
            ...
            if (value !== this.cacheDispatch[this.activeAdvanceIndex].type) {
                ...
                this.advanceForm.dispatchParams = this.cacheDispatch[
        ...
        async advanceDispatch() {
            ...
            let res = await ngxDispatch(this.advanceData, {
                app_service_id: this.$route.params.app,
            });
