<template>
    <my-form-item :title="title" @closeConfig="closeConfig" @saveConfig="saveConfig" @cancel="cancel" @edit="edit" :open="true" :modify="modify" :valid="valid" :important="true" :info="info">
        <div slot="edit">
            <draggable v-model="form.ngcUpstreamServers" handle=".drag-handle" @start="drag = true" ghost-class="drag-item-ghost" @end="drag = false">
                <transition-group>
                    <div v-for="(item, index) in form.ngcUpstreamServers" :key="index">
                            ...
                            <div class="item-body">
                                <Form ref="form" :model="item" :rules="formRules" @submit.native.prevent>
                                    ...
                                    <FormItem label="RESOLVE" class="aline-center common-color">
                                        ...
                                        <PopTip style="margin-left: 10px;margin-top: 2px;" :size="18" content="若要使此工作正常，应在upstream group内设置共享内存区域。此外，还必须在http块中指定resolver指令。" placement="right"></PopTip>
                                    </FormItem>
...
export default {
    ...
    data() {
        ...
        return {
            title: "server",
            info:
                "指定upstream group中的后端服务器。服务器由IP地址或FQDN标识。可以指定可选端口，默认端口为80。",
            formRules: {
    ...
    methods: {
        ...
        addRule() {
            let json = this.copyJson(
                emptyConfig.ngcUpstreamGroups[0].ngcUpstreamServers[0]
            );
