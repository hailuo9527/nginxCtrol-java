            ...
            <div class="content_right_scroll">
                <div class="content_header">
                    <div class="header_title">
                        <span>
                            ...
                            <Drawer :title="activeAside.app_service_name" width="420" placement="left" class="left-drawer" :closable="false" v-model="openDrawer">
                                <div class="content">
                                    <div class="row-item">
                                        <div class="status">
                                    ...
                                    <row-item title="入口URL">
                                        <div class="row-item" v-if="detailInfo.urls">
                                            <div class="info-list">
                                                ...
                                                <div class="list-item" :key="item.l7ServerId" v-for="item in detailInfo.urls">
                                                    <Row>
                                                        <Col span="18">
                                                        <span>URL： {{ item }}</span>
                                                        </Col>
                                    ...
                                    <row-item title="后端服务器">
                                        <div class="row-item" v-if="detailInfo.appUpServerBlockList">
                                            <div class="info-list">
                                                <div class="list-item no-item" v-if="!detailInfo.appUpServerBlockList.length">
                                                    暂无数据
                                                </div>
                                                <div class="list-item" v-else :key="item.l7ServerId" v-for="item in detailInfo.appUpServerBlockList">
                                                    <Row>
                                                        <Col span="18">
                        ...
                        <div style="display: inline-block; margin-left: 10px" class="Pop-tip">
                            <Icon type="md-git-compare" size="20" style="cursor: pointer" v-if="!activeAside.is_sync" @click="showModal()" />
                            <Modal v-model="compareModal" :mask="true" width="80" :mask-closable="false">
                                <span slot="close"></span>
                                <div v-if="!Compareloading">
                                    <div slot="close" style="text-align: right;margin: -10px -6px 10px 0;">
                                        <Icon type="md-close" size="22" style="cursor: pointer;" @click="compareModal = false" />
                                    </div>
                                    <code-diff :old-string="oldStr" :new-string="newStr" :context="1000" outputFormat="side-by-side" v-if="popTip_status" />
                                    <div v-if="!popTip_status" style="font-size: 18px;padding: 10px 10px 10px 50px">
                                        您还没修改过APP配置或还未发布
                                    </div>
                                </div>
                                ...
                                <div slot="footer">
                                    <Button @click="compareModal = false" style="margin-right: 10px">取消</Button>
                                    <Poptip confirm title="是否确定回滚?" ok-text="确定" @on-ok="RollBack" cancel-text="取消" style="color: #000">
                                        <Button type="primary" :disabled="!popTip_status">回滚</Button>
                                    </Poptip>
                                </div>
                            </Modal>
                        </div>
              ...
              >
                快捷发布
                            ...
                            <Button style="margin-left: 20px" @click="pushApp" type="primary">发布</Button>
                            <Modal v-model="appModal" width="480">
                                ...
                                <div slot="footer">
                                    ...
                                    <Button type="primary" @click="pushApp('formValidate')">确认</Button>
                                </div>
                            </Modal>
                        </span>
                    </div>
                    <div class="header_tab">
                        <div class="tab">
                            ...
                            <router-link v-if="activeAside.configure_ha" :to="`/app/${$route.params.app}/ha`" class="tab_item">HA</router-link>
                            <span v-else class="tab_item disabled">HA</span>
                            <router-link :to="`/app/${$route.params.app}/config`" class="tab_item">配置</router-link>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        /* 发布APP */
        pushApp() {
            let temp = this.activeAside.appUpgroupBlockList.map(function (
                item
            ) {
                            ...
                            render: (h) => {
                                return h("div", [
                                    ...
                                    h("p", {
                                        domProps: {
                                            innerHTML:
                                                res.data.result.plus_conf_param,
                                        },
                                        style: {
                                            color: "#333",
                                            fontSize: "14px",
                                    ...
                                    h("div", [
                                        h("span", "所选实例中"),
                                        res.data.result.l7ServerName.map(
                                            (item) => {
                                                return h("span", {
                                                    class: "l7ServerName",
                                                    domProps: {
                        ...
                        switch (i) {
                            case "configure_ha":
                                this.newStr = this.newStr.concat(
                                    "热备状态:" +
                                        res.data.result[i].new_str +
                                        "\n"
                                );
                                this.oldStr = this.oldStr.concat(
                                    "热备状态:" +
                                        res.data.result[i].old_str +
                                        "\n"
                                );
                                break;
                            case "app_vip":
                            ...
                            case "app_server_block":
                                for (
                                    ...
                                    if (
                                        res.data.result[i].new_str[newnum]
                                            .appLocationBlockList.length > 0
                                    ) {
                                        for (
                                            let newLocationnum = 0;
                                            newLocationnum <
                                            res.data.result[i].new_str[newnum]
                                                .appLocationBlockList.length;
                                            newLocationnum++
                                        ) {
                                            this.newStr = this.newStr.concat(
                                                "proxy_url:" +
                            ...
                            case "app_group_block":
                                for (
                                    ...
                                    if (
                                        res.data.result[i].new_str[newnum]
                                            .appUpServerBlockList.length > 0
                                    ) {
                                        for (
                                            let newUpServernum = 0;
                                            newUpServernum <
                                            res.data.result[i].new_str[newnum]
                                                .appUpServerBlockList.length;
                                            newUpServernum++
                                        ) {
                                            this.newStr = this.newStr.concat(
                                                "应用服务成员:" +
