        ...
        <div class="content" style="margin-top: 0px;">
            <div class="overview-container overview-container_css-grid">
                ...
                <div class="overview-container__content">
                    <div class="overview-score-box" :class="{
              ...
              'overview-box_unhealthy': prewarn_unhealthy,
                  ...
                  common_color_white: is_white,
                            ...
                            <div class="overview-box__l">
                                <h4 class="overview-box__value-title_prewarn" :class="{
                    common_color_black: is_black,
                    common_color_white: is_white,
                  }">
                                    CPU预警数量
                                ...
                                <span class="overview-box__l__content">
                                    <span class="overview-box__l__value" :class="{
                      common_color_black: is_black,
                      common_color_white: is_white,
                ...
                <span class="overview-box__l__unit"></span> -->
                    ...
                    common_color_white: is_white,
                                ...
                                <span class="overview-box__r__value" :class="{
                    common_color_black: is_black,
                    common_color_white: is_white,
                  }">{{ diskWarningCount }}</span>
                            </div>
                        </div>
                        <div class="overview-box-l-bottom-line" :class="{
                ...
                'overview-box-r-bottom-line_unhealthy': bottom_line_unhealthy,
                        ...
                        <div class="overview-box-r-bottom-line" :class="{
                'overview-box-r-bottom-line_healthy': bottom_line_healthy,
                'overview-box-r-bottom-line_sub_healthy': bottom_line_subhealthy,
                'overview-box-r-bottom-line_unhealthy': bottom_line_unhealthy,
              }"></div>
                        <div class="prewarn-footer-l">
                            ...
                            <span v-if="cpuWarning.length == 0" :class="{
                  common_color_black: is_black,
                  common_color_white: is_white,
                }">暂无CPU预警</span>
                            <span v-else v-for="item in cpuWarning">
                                <span class="l-text-position high-light" v-if="item.value <= 20">{{ item.name }}</span>
                  ...
                  item.name
                        ...
                        <div class="prewarn-footer-r">
                            <span v-if="diskWarning.length == 0" :class="{
                  common_color_black: is_black,
                  common_color_white: is_white,
                }">暂无磁盘预警</span>
                            <span v-else v-for="item in diskWarning">
                                <span class="r-text-position high-light" v-if="item.value <= 10">{{ item.name }}</span>
                  ...
                  item.name
                    ...
                    <chart-box :data="item" :index="index" v-if="chartData.length" :key="index" v-for="(item, index) in chartData" @remove="RemoveApp"></chart-box>
                    <div class="overview-box">
                        <div class="add-button" @click="DisplayModel()">+</div>
                    </div>
                    <!-- 添加app的Modal -->
                    <Modal v-model="AddModel" width="30" :mask-closable="false">
                        <div slot="header">添加APP</div>
                        <div style="padding:1rem;">
                            <div style="color: #aaa; margin-bottom: 20px">
                                请选择想要添加到首页的APP
                            </div>
                            <Select v-model="SelectModel" style="width:100%" @on-select="select">
                                <Option v-for="item in unusedApp" :value="item.app_service_id" :key="item.app_service_id">{{ item.app_service_name }}</Option>
                            </Select>
                        </div>

                        <div slot="footer">
                            <Button type="primary" :long="true" @click="handleSubmit()">确认添加</Button>
                        </div>
                    </Modal>
                </div>
                <div class="home-spin">
            ...
            if (res.data.code === "success") {
                ...
                if (res.data.result == null) {
                    ...
                    if (data.instancePercent == 1) {
                        ...
                        data.instancePercent < 1 &&
                        data.instancePercent >= 0.4
                    ) {
