<template>
  <div class="config-list">
      ...
      >
        <template slot-scope="{ row, index }" slot="action">
          ...
          <Icon
            ...
            @click.stop="remove(row.nginx_conf_id, index)"
          />
          <Icon
            type="md-lock"
            size="22"
            style="cursor: pointer; color: #f16643"
            v-if="row.lock_status"
            @click.stop="LockAndUnlock(row)"
          />
          <Icon
            type="md-unlock"
            size="22"
            style="cursor: pointer; color: #01c864"
            v-if="!row.lock_status"
            @click.stop="LockAndUnlock(row)"
          />
        </template>
    ...
    >
      ...
      <div>
        <span
          style="display: block; float: left; line-height: 32px; margin-right: 10px;"
          >文件导入</span
        >
        <Upload :before-upload="handleUpload" action="/url">
          <Button icon="ios-cloud-upload-outline">导入文件</Button>
        </Upload>
      </div>
      <div v-if="file !== null" class="file-content">
        <span>{{ file.name }}</span
        ><Icon type="md-close" class="close-icon" @click="close()" />
      </div>
      <div class="footer" slot="footer">
        <Button @click="cancel()">取消</Button>
        <Button type="primary" @click="upload()" :loading="loadingStatus">{{
          loadingStatus ? "导入中..." : "导入"
        }}</Button>
      </div>
    </Modal>
  </div>
...
export default {
  data() {
    return {
      ...
      ruleInline: {
        inputValue: [{ required: true, message: "不能为空", trigger: "blur" }],
      },
      file: null,
      loadingStatus: false,
    };
      ...
      this.$refs.formInline.validate(async (valid) => {
        if (valid) {
          if (this.file !== null) {
            ...
            let res = await uploadNgConf(file, {
              config_name: this.formInline.inputValue,
            });
            if (res.data.code === "200") {
              this.loadingStatus = false;
              this.importModal = false;
              this.$Message.success({
                content: "文件 " + this.file.name + "导入成功！",
                duration: 3,
              });
              this.file = null;
              ...
              this.$Message.error({
                content:
                  "文件 " + this.file.name + "导入失败！" + res.data.result,
                duration: 3,
              });
              this.file = null;
            }
