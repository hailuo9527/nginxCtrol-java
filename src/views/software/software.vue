<template>
    <div class="config-list">
        <div class="config_header">
            ...
            <Modal v-model="uploadModal" :mask-closable="false" width="40">
                <div slot="header">文件上传</div>
                <div>
                    <Form>
                        <FormItem label='服务器版本'>
                            <Select v-model="versionId">
                                <Option v-for="(item,index) in versionList" :value="item.operation_system_verion_id" :key="index">{{ item.operation_system_verion_name }}</Option>
                            </Select>
                        </FormItem>
                        <FormItem label="文件上传">
                            ...
                            <Row v-for="(item,index) in file" :Key="index" v-if="file.length">
                                <div class="ivu-upload-list-file">
                                    ...
                                    {{ item.name }}
                                    <Icon v-show="showRemoveFile" type="ios-close" size="32" class="ivu-upload-list-remove" @click.native="handleRemove(index)"></Icon>
                                </div>
                            </Row>
                            <!--<Row>
        ...
        <div class="config_table_wrapper">
            <Table :columns="tableConfig" :loading="loading" :data="tableData">
                <template slot-scope="{ row, index }" slot="action">
                    ...
                    <Poptip transfer confirm title="确定要删除这个文件吗？" @on-ok="delFile(row)">
        ...
        <Modal v-model="installModal" title="isOnline?'在线安装':'软件安装'" width="60" :closable="false" :mask-closable="false">
            <div slot='header'>
                ...
                <Tooltip content="请输入需要安装的软件名称（譬如：nginx）,再选择需要安装的目标实例。确定即可" :transfer="true" theme="light" v-if="isOnline" max-width='500px' placement='right'>
                    <Icon class="handle" size="20" type="md-information-circle" color="#01c864" />
                </Tooltip>
    ...
    methods: {
        ...
        async delFile(row) {
            let res = await delFile({
                path: row.file_path,
                version: row.operation_system_verion_id,
            });
            console.log(res);
            if (this.asyncOk(res)) {
        ...
        async installNginx() {
            if (this.selectedServer.length > 0) {
                ...
                let res = await installNginx(this.selectedServer, {
                    filePath: this.active.file_path,
                });
        ...
        async upload() {
            ...
            let promises = this.file.map((item, index) => {
                ...
                return uploadFile({
                    file: fileFormData,
                    operation_system_verion_id: this.versionId,
                }).then((res) => {
                    if (res.data.code === "success") {
                        ...
                        this.$Message.error(
                            "文件 " + item.name + "上传失败！" + res.data.result
                        );
                    }
                });
            });
